...
int main(int argc, char *argv[]) {
    ...
    // LARGE_INTEGER startingTime, endingTime, millisec, freq;
    ...
    // millisec.QuadPart = endingTime.QuadPart - startingTime.QuadPart;
    // millisec.QuadPart *= 1000;
    // millisec.QuadPart /= freq.QuadPart;
    // fprintf(stderr, "(%d edges, operation took %d ms)\n", graph->size, millisec.QuadPart);
    ...
    if (opts.verify) {
        ...
        if (verify(graph, sorted)) {
            printf("Valid topological ordering\n");
        } else {
            printf("Invalid topological ordering\n");
        }
    }
